...
def run_one_denoising_step(pipeline, config, prompt, n_prompt, fps_condition, seed, steps, scale, frames, size, shift, capture_storage=None):
    """
    Run ONE denoising step to capture ||y_text|| magnitudes at each FPS block.

    Args:
        capture_storage: Dict to store {block_idx: ||y_text||}

    Note: The pipeline should already have the desired checkpoint loaded (clean, base_only, or full)
    """
    ...
    # Hook to capture ||y_text|| from each FPS block
    captured_magnitudes = {}
    ...
    def make_capture_hook(block_idx):
        def hook(module, args, kwargs, output):
            ...
            # We need to capture y_text, which is computed from k_text, v_text
            # Since we can't easily extract y_text from output, we compute it ourselves
            if len(args) >= 3:
                ...
                with torch.no_grad():
                    ...
                    captured_magnitudes[block_idx] = y_text_norm
    ...
    # Register hooks on FPS adapter blocks AFTER creating layers
    ...
    for block_idx, block in enumerate(transformer_model.blocks):
        if hasattr(block, 'fps_adapter') and block.fps_adapter is not None:
            ...
            # Hook directly on the FPS adapter's forward method
...
def calibrate_alignment_ratios_onestep(pipeline, config, checkpoint_path, prompt, n_prompt, fps_condition, seed, steps, scale, frames, size, shift):
    """
    ONE-STEP TEXT MAGNITUDE ALIGNMENT ALGORITHM (ALIGN_INFERNCE_TEXT_ONESTEP.md):

    Subtask 1: Log ratios for each FPS adapter-injected block i:
        ratio_i = ||y_text_base_lora_only|| / ||y_text_clean||

    This is done by running ONE denoising step (not full inference):
    1. Run one step with CLEAN backbone (reload without any checkpoint) → capture ||y_text_clean||_i per block
    2. Reload with BASE LoRA ONLY → Run one step → capture ||y_text_base_lora||_i per block
    3. Compute ratio_i for each block i

    Returns: ratio_dict mapping {block_idx: ratio_i}
    """
    logging.info("🔧 Running ONE-STEP TEXT MAGNITUDE alignment calibration...")
    logging.info("  Will run ONE denoising step TWICE with different checkpoint loading modes")
    logging.info("  Capturing ||y_text|| magnitudes at each FPS-injected block")

    print(f"\n{'='*80}", flush=True)
    print(f"🔧 CALIBRATION PHASE: One-step magnitude comparison", flush=True)
    print(f"{'='*80}", flush=True)

    # Storage for magnitudes
    clean_magnitudes = {}  # {block_idx: ||y_text_clean||}
    base_lora_magnitudes = {}  # {block_idx: ||y_text_base_lora||}

    # Step 1: Run one step with CLEAN backbone (no checkpoint loaded)
    print(f"\n  [1/2] Running one step with CLEAN backbone (no LoRA)...", flush=True)
    ...
    # Step 2: Load BASE LoRA ONLY from checkpoint
    print(f"\n  Loading BASE LoRA ONLY from checkpoint...", flush=True)
    base_rank = config.get('adapter', {}).get('rank', 32)
    apply_checkpoint(pipeline, checkpoint_path, rank=base_rank, base_only=True, config=config)

    # Run one step with BASE LoRA ONLY
    print(f"\n  [2/2] Running one step with BASE LoRA ONLY (no FPS)...", flush=True)
    run_one_denoising_step(
        pipeline, config, prompt, n_prompt, fps_condition, seed, steps, scale, frames, size, shift,
        capture_storage=base_lora_magnitudes
    )

    # Step 3: Compute ratios
    ratio_dict = {}
    print(f"\n{'='*80}", flush=True)
    print(f"📊 CALIBRATION RESULTS: Magnitude Ratios per Block", flush=True)
    print(f"{'='*80}", flush=True)
    print(f"{'Block':<10} {'||clean||':<15} {'||base_lora||':<15} {'Ratio':<15}", flush=True)
    print(f"{'-'*55}", flush=True)

    for block_idx in sorted(clean_magnitudes.keys()):
        clean_mag = clean_magnitudes[block_idx]
        base_lora_mag = base_lora_magnitudes.get(block_idx, 0.0)
        ratio = base_lora_mag / (clean_mag + 1e-8)
        ratio_dict[block_idx] = ratio
        print(f"{block_idx:<10} {clean_mag:<15.4f} {base_lora_mag:<15.4f} {ratio:<15.4f}", flush=True)

    print(f"{'='*80}", flush=True)
    print(f"✅ Calibration complete: Captured {len(ratio_dict)} block ratios", flush=True)
...
def generate_video_with_fps(pipeline, config, prompt, n_prompt, fps=60, seed=42, steps=25, scale=7.0, frames=49, size=(512, 320), shift=3.0, force_gate_one=False, alignment_ratios=None, capture_rboth=False, rboth_storage=None, fps_idx=None, apply_onestep_alignment=False):
    ...
    PARAMS for ONE-STEP alignment (ALIGN_INFERNCE_TEXT_ONESTEP.md):
    - apply_onestep_alignment: If True, scale gates using alignment_ratios
    - alignment_ratios: Dict {block_idx: ratio_i} where ratio_i = ||y_text_base_lora|| / ||y_text_clean||
    ...
    if capture_rboth or alignment_ratios is not None:
        ...
        def make_fps_wrapper(original_forward, block_idx, step_idx_ref):
            ...
            def wrapper(q, k_text, v_text, fps_conditioning, context_lens):
                ...
                # MODE 1: ONE-STEP ALIGNMENT - Scale gate by ratio (ALIGN_INFERNCE_TEXT_ONESTEP.md)
                if apply_onestep_alignment and alignment_ratios is not None:
                    # Subtask 2: Apply alignment by scaling gate
                    # new_gate_i = old_gate_i * ratio_i
                    if block_idx in alignment_ratios:
                        ratio_i = alignment_ratios[block_idx]
                        ...
                        if step_i == 0:
                            ...
                            if block_idx in sorted_blocks[:10]:  # Show first 10 blocks
                                ...
                                logging.info(f"  🎯 [Block {block_idx}] ONE-STEP ALIGNMENT: "
                                           f"gate {original_gate:.4f} × ratio {ratio_i:.4f} = {scaled_gate:.4f}")
