#!/usr/bin/env python3
"""
GRAFT + ALIGNMENT inference script - Load base LoRA only in FPS adapter blocks with alignment.

GRAFT + ALIGNMENT APPROACH (updated /root/workspace/sc-diffusion-pipe/plans/GRAFT.md):
1. GRAFT: Load base LoRA only in blocks where FPS adapters exist (e.g., blocks 27-39)
2. ALIGNMENT: Calibrate magnitude ratio comparing:
   - ||y_text_base_lora_full|| (full base LoRA on all blocks)
   - ||y_text_base_lora_partial|| (grafted base LoRA on FPS blocks only)
3. Apply scaling: new_gate = old_gate × (||y_text_full|| / ||y_text_partial||)

This approach:
- Reduces interference from base LoRA in non-FPS blocks (GRAFT)
- Compensates for reduced base LoRA magnitude with alignment scaling
- Combines spatial matching with magnitude calibration
"""
...
def load_adapter_weights_selective(pipeline, checkpoint_path, fps_only=False, base_only=False, graft_mode=False, fps_block_indices=None):
    ...
    Args:
        graft_mode: If True, only load base LoRA for blocks specified in fps_block_indices
        fps_block_indices: List of block indices where FPS adapters exist (for GRAFT mode)
    """
    ...
    if fps_only:
        ...
        for k, v in checkpoint.items():
            ...
            # For base LoRA: only include if it's in one of the FPS blocks
                ...
                # Base LoRA parameter format: diffusion_model.blocks.{block_idx}.{rest}
                # Extract block index from parameter name
                if '.blocks.' in k:
                    try:
                        # Parse: diffusion_model.blocks.27.cross_attn.k.lora_A.default.weight
                        parts = k.split('.blocks.')[1].split('.')
                        block_idx = int(parts[0])
                        if block_idx in fps_block_indices:
                            filtered_checkpoint[k] = v
                    except (IndexError, ValueError):
                        # Skip parameters that don't match expected format
                        pass

        mode_name = "GRAFT"
        logging.info(f"🌿 {mode_name} MODE: Filtered {len(filtered_checkpoint)} parameters (FPS + grafted base LoRA) out of {len(checkpoint)} total")
    ...
    if fps_only:
        ...
        # GRAFT mode: show detailed breakdown
...
def apply_checkpoint(wan_t2v_pipeline, checkpoint_path, rank=32, dtype=torch.bfloat16, fps_only=False, base_only=False, graft_mode=False, config=None):
    ...
    Args:
        graft_mode: If True, load FPS adapters + base LoRA only in FPS adapter blocks
    """
    ...
    if configure_base_lora:
        # Check if adapter is already configured (e.g., from previous calibration step)
        if hasattr(wan_t2v_pipeline.transformer, 'peft_config') and wan_t2v_pipeline.transformer.peft_config:
            logging.info(f"Base LoRA adapter already configured (from previous load), skipping configuration")

            # CRITICAL FIX: Reset all base LoRA parameters to zero before loading new weights
            # This ensures GRAFT mode loads properly on top of FULL base LoRA
            if graft_mode:
                logging.info(f"🌿 GRAFT MODE: Resetting existing base LoRA weights to zero before loading grafted weights")
                for name, param in wan_t2v_pipeline.transformer.named_parameters():
                    # Reset base LoRA parameters (lora_A, lora_B), but NOT FPS parameters
                    if ('lora_A' in name or 'lora_B' in name) and 'fps' not in name:
                        param.data.zero_()
                logging.info(f"   ✅ Reset base LoRA weights to zero")
        else:
...
def calibrate_graft_alignment_ratios(pipeline, config, checkpoint_path, prompt, n_prompt, fps_condition, seed, steps, scale, frames, size, shift):
    """
    GRAFT ALIGNMENT CALIBRATION (updated /root/workspace/sc-diffusion-pipe/plans/GRAFT.md):

    Compare magnitude when using FULL base LoRA vs PARTIAL (grafted) base LoRA:
        ratio_i = ||y_text_base_lora_full|| / ||y_text_base_lora_partial||

    Steps:
    1. Run one step with BASE_LORA_FULL (all blocks) + FPS → capture ||y_text_full||
    2. Run one step with BASE_LORA_PARTIAL (grafted, FPS blocks only) + FPS → capture ||y_text_partial||
    3. Compute ratio_i = ||y_text_full|| / ||y_text_partial|| for each block

    NOTE: The ratio is INDEPENDENT of fps_condition - it measures the structural difference
    between full vs partial base LoRA, not the FPS conditioning effect. We use fps_condition
    only as a reference value for the denoising step (any value would work).

    Returns: ratio_dict mapping {block_idx: ratio_i}
    """
    logging.info("🔧🌿 Running GRAFT ALIGNMENT calibration...")
    logging.info("  Comparing FULL base LoRA vs PARTIAL (grafted) base LoRA magnitudes")
    logging.info("  NOTE: Ratio is FPS-condition-independent (structural difference only)")
    logging.info("  Will run ONE denoising step TWICE with different base LoRA loading modes")

    print(f"\n{'='*80}", flush=True)
    print(f"🔧🌿 CALIBRATION PHASE (GRAFT): Full vs Partial base LoRA comparison", flush=True)
    print(f"   NOTE: Ratio is independent of FPS condition (using FPS={fps_condition} as reference)", flush=True)
    print(f"{'='*80}", flush=True)

    # Storage for magnitudes
    full_magnitudes = {}     # {block_idx: ||y_text|| with full base LoRA}
    partial_magnitudes = {}  # {block_idx: ||y_text|| with partial/grafted base LoRA}

    base_rank = config.get('adapter', {}).get('rank', 32)

    # Step 1: Run one step with FULL base LoRA + FPS adapter
    print(f"\n  [1/2] Running one step with FULL base LoRA (all blocks) + FPS adapter...", flush=True)
    apply_checkpoint(pipeline, checkpoint_path, rank=base_rank, fps_only=False, base_only=False, graft_mode=False, config=config)

    run_one_denoising_step(
        pipeline, config, prompt, n_prompt, fps_condition, seed, steps, scale, frames, size, shift,
        capture_storage=full_magnitudes
    )

    # Step 2: Run one step with PARTIAL (grafted) base LoRA + FPS adapter
    print(f"\n  [2/2] Running one step with PARTIAL/GRAFTED base LoRA (FPS blocks only) + FPS adapter...", flush=True)
    apply_checkpoint(pipeline, checkpoint_path, rank=base_rank, fps_only=False, base_only=False, graft_mode=True, config=config)

    run_one_denoising_step(
        pipeline, config, prompt, n_prompt, fps_condition, seed, steps, scale, frames, size, shift,
        capture_storage=partial_magnitudes
    )

    # Step 3: Compute ratios
    ratio_dict = {}
    print(f"\n{'='*80}", flush=True)
    print(f"📊 CALIBRATION RESULTS (GRAFT): Magnitude Ratios per Block", flush=True)
    print(f"   FPS Condition = {fps_condition}", flush=True)
    print(f"{'='*80}", flush=True)
    print(f"{'Block':<10} {'||y_text_full||':<18} {'||y_text_partial||':<18} {'Ratio (full/partial)':<20}", flush=True)
    print(f"{'-'*70}", flush=True)

    for block_idx in sorted(full_magnitudes.keys()):
        # Extract y_text magnitude from the dict (hook captures both y_text and y_fps)
        full_data = full_magnitudes[block_idx]
        partial_data = partial_magnitudes.get(block_idx, {'y_text': 0.0, 'y_fps': 0.0})

        full_mag = full_data.get('y_text', full_data) if isinstance(full_data, dict) else full_data
        partial_mag = partial_data.get('y_text', partial_data) if isinstance(partial_data, dict) else partial_data

        ratio = full_mag / (partial_mag + 1e-8)
        ratio_dict[block_idx] = ratio
        print(f"{block_idx:<10} {full_mag:<18.4f} {partial_mag:<18.4f} {ratio:<20.4f}", flush=True)

    print(f"{'='*80}", flush=True)
    print(f"✅ GRAFT calibration complete for FPS={fps_condition}: Captured {len(ratio_dict)} block ratios", flush=True)

    # Show summary statistics
    ratio_values = list(ratio_dict.values())
    print(f"\n📈 Summary Statistics:", flush=True)
    print(f"   Ratio: mean={sum(ratio_values)/len(ratio_values):.4f}, min={min(ratio_values):.4f}, max={max(ratio_values):.4f}", flush=True)
...
def generate_video_with_fps(pipeline, config, prompt, n_prompt, fps=60, seed=42, steps=25, scale=7.0, frames=49, size=(512, 320), shift=3.0, force_gate_one=False, alignment_ratios=None, capture_rboth=False, rboth_storage=None, fps_idx=None, apply_onestep_alignment=False):
    ...
    PARAMS for GRAFT ALIGNMENT (updated /root/workspace/sc-diffusion-pipe/plans/GRAFT.md):
    - apply_onestep_alignment: If True, scale gates using alignment_ratios
    - alignment_ratios: Dict {block_idx: ratio_i} where:
        ratio_i = ||y_text_base_lora_full|| / ||y_text_base_lora_partial||
      Applied as: new_gate_i = old_gate_i * ratio_i

    Also supports PLUS alignment with dict format {block_idx: {'r_text': ..., 'r_cond': ...}}
...
def run_fps_experiments(pipeline, config, base_prompt, n_prompt, fps_values, output_dir, checkpoint_path=None, force_gate_one=False, align=False, debug_gate_scale=None, **generation_kwargs):
    ...
    if align:
        ...
        if debug_gate_scale is not None:
            ...
            # Load checkpoint with GRAFT mode
            logging.info(f"\n🔧🌿 Loading checkpoint in GRAFT mode (debug mode inference)...")
            base_rank = config.get('adapter', {}).get('rank', 32)
            apply_checkpoint(pipeline, checkpoint_path, rank=base_rank, graft_mode=True, config=config)
            logging.info(f"   ✅ Loaded GRAFTED base LoRA + FPS adapters\n")
            ...
            alignment_ratios = calibrate_graft_alignment_ratios(
            ...
            if alignment_ratios:
                # ⚠️ CRITICAL: After GRAFT calibration, we need to reload pipeline fresh
                # During calibration, we loaded FULL base LoRA (all blocks) and then PARTIAL base LoRA (FPS blocks)
                # We MUST reload the entire pipeline fresh to have clean state for GRAFT inference
                print(f"\n🔧 CRITICAL: Reloading pipeline FRESH after GRAFT calibration...", flush=True)
                print(f"   Reason: Calibration loaded both FULL and PARTIAL base LoRA configurations", flush=True)
                print(f"   Need clean pipeline to apply GRAFT mode with alignment", flush=True)
                ...
                # Now load checkpoint with GRAFT mode (partial base LoRA + FPS adapters) for inference
                print(f"\n🔧🌿 Loading checkpoint in GRAFT mode for aligned inference...", flush=True)
                base_rank = config.get('adapter', {}).get('rank', 32)
                apply_checkpoint(pipeline, checkpoint_path, rank=base_rank, graft_mode=True, config=config)
                print(f"   ✅ Inference will use GRAFTED base LoRA + FPS adapters with alignment scaling\n", flush=True)
...
def get_fps_block_indices(config):
    ...
    # Parse fps_condition_blocks (e.g., "default", "27-39", "[27,28,29]")
    if fps_condition_blocks == 'default':
