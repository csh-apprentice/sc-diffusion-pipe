#!/usr/bin/env python3
"""
GRAFT inference script - Load base LoRA only in FPS adapter blocks.

GRAFT APPROACH (from /root/workspace/sc-diffusion-pipe/plans/GRAFT.md):
Instead of loading base LoRA on ALL blocks, only load it in the SAME blocks where
FPS adapters are injected. For example, if FPS adapters are in the deepest third
(blocks 27-39), only load base LoRA for those specific blocks.

This "grafts" the base LoRA spatial pattern to match the FPS adapter pattern,
reducing interference from base LoRA in blocks without FPS adapters.

Key differences from standard inference:
- Identifies which blocks have FPS adapters
- Filters base LoRA checkpoint to only include parameters for those blocks
- Combines base LoRA (grafted) + FPS adapters for inference
"""
