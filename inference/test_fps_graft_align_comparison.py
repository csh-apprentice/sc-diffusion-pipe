#!/usr/bin/env python3
"""
GRAFT + ALIGNMENT inference script - COMPARISON STUDY VERSION

This is a comparison study that changes the calibration ratio while keeping everything else the same:

COMPARISON APPROACH:
1. GRAFT: Load base LoRA only in blocks where FPS adapters exist (e.g., blocks 27-39) [SAME]
2. ALIGNMENT CALIBRATION: Compare FULL base LoRA vs CLEAN backbone [CHANGED]
   - ratio = ||y_text_base_lora_full|| / ||y_text_clean||
   - Original approach used: ||y_text_full|| / ||y_text_partial||
3. INFERENCE: Use GRAFT mode (partial base LoRA + FPS adapters) [SAME]
4. Apply scaling: new_gate = old_gate × ratio [SAME]

This allows comparing:
- Original GRAFT: Calibrate with full vs partial, infer with partial
- Comparison: Calibrate with full vs clean, infer with partial
"""
...
def calibrate_graft_alignment_ratios(pipeline, config, checkpoint_path, prompt, n_prompt, fps_condition, seed, steps, scale, frames, size, shift):
    """
    COMPARISON STUDY - GRAFT ALIGNMENT CALIBRATION:

    Compare magnitude when using FULL base LoRA vs CLEAN backbone:
        ratio_i = ||y_text_base_lora_full|| / ||y_text_clean||

    Steps:
    1. Run one step with CLEAN backbone + FPS → capture ||y_text_clean||
    2. Run one step with BASE_LORA_FULL (all blocks) + FPS → capture ||y_text_full||
    3. Compute ratio_i = ||y_text_full|| / ||y_text_clean|| for each block

    NOTE: This is a COMPARISON STUDY. We calibrate with full vs clean, but still
    use GRAFT mode (partial base LoRA) for inference.

    Returns: ratio_dict mapping {block_idx: ratio_i}
    """
    logging.info("🔧🌿 Running GRAFT ALIGNMENT calibration (COMPARISON STUDY)...")
    logging.info("  Comparing FULL base LoRA vs CLEAN backbone magnitudes")
    logging.info("  COMPARISON: ratio = ||y_text_full|| / ||y_text_clean||")
    logging.info("  Will run ONE denoising step TWICE with different loading modes")

    print(f"\n{'='*80}", flush=True)
    print(f"🔧🌿 CALIBRATION PHASE (COMPARISON STUDY): Full base LoRA vs Clean backbone", flush=True)
    print(f"   Calibration ratio: ||y_text_full|| / ||y_text_clean||", flush=True)
    print(f"   Inference mode: GRAFT (partial base LoRA + FPS adapters)", flush=True)
    print(f"{'='*80}", flush=True)

    # Storage for magnitudes
    clean_magnitudes = {}    # {block_idx: ||y_text|| with clean backbone + FPS}
    full_magnitudes = {}     # {block_idx: ||y_text|| with full base LoRA + FPS}

    base_rank = config.get('adapter', {}).get('rank', 32)

    # Step 1: Run one step with CLEAN backbone + FPS adapter (no base LoRA)
    print(f"\n  [1/2] Running one step with CLEAN backbone + FPS adapter (no base LoRA)...", flush=True)
    ...
    run_one_denoising_step(
        ...
        capture_storage=clean_magnitudes
    )

    # Step 2: Run one step with FULL base LoRA + FPS adapter
    print(f"\n  [2/2] Running one step with FULL base LoRA (all blocks) + FPS adapter...", flush=True)
    ...
    ratio_dict = {}
    print(f"\n{'='*80}", flush=True)
    print(f"📊 CALIBRATION RESULTS (COMPARISON STUDY): Magnitude Ratios per Block", flush=True)
    print(f"   FPS Condition = {fps_condition}", flush=True)
    print(f"{'='*80}", flush=True)
    print(f"{'Block':<10} {'||y_text_clean||':<18} {'||y_text_full||':<18} {'Ratio (full/clean)':<20}", flush=True)
    print(f"{'-'*70}", flush=True)

    for block_idx in sorted(clean_magnitudes.keys()):
        # Extract y_text magnitude from the dict (hook captures both y_text and y_fps)
        clean_data = clean_magnitudes[block_idx]
        full_data = full_magnitudes.get(block_idx, {'y_text': 0.0, 'y_fps': 0.0})

        clean_mag = clean_data.get('y_text', clean_data) if isinstance(clean_data, dict) else clean_data
        full_mag = full_data.get('y_text', full_data) if isinstance(full_data, dict) else full_data

        ratio = full_mag / (clean_mag + 1e-8)
        ratio_dict[block_idx] = ratio
        print(f"{block_idx:<10} {clean_mag:<18.4f} {full_mag:<18.4f} {ratio:<20.4f}", flush=True)

    print(f"{'='*80}", flush=True)
    print(f"✅ COMPARISON STUDY calibration complete for FPS={fps_condition}: Captured {len(ratio_dict)} block ratios", flush=True)
