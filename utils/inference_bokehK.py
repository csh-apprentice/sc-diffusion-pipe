...
class Camera_Embedding(Dataset):
    ...
    def load(self):
        # Remove hard restriction on 5 values if you want flexible frames
        ...
        # --- FIX START ---
        # 1. Always reshape to the native resolution first (256x384 matches the text embedding size)
        ...
        # 3. Interpolate (resize) to the target sample_size (e.g., 512x512)
        ...
        # Create bokeh embedding at the TARGET size
...
def run_inference(pipeline, tokenizer, text_encoder, base_scene, bokehK_list, output_dir, device, video_length=5, height=512, width=512):
    ...
    camera_embedding = Camera_Embedding(bokehK_values, tokenizer, text_encoder, device, sample_size=[height, width]).load()
