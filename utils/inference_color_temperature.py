...
class Camera_Embedding(Dataset):
    ...
    def load(self):
        # Comment this out if you want to use fewer/more than 5 frames
        # if len(self.color_temperature_values) != 5:
        #    raise ValueError("Expected 5 color_temperature values")

        # Generate prompts for each color_temperature value
        prompts = []
        ...
        if pad_length > 0:
            concatenated_differences_padded = F.pad(concatenated_differences, (0, 0, 0, pad_length))
             ...
             concatenated_differences_padded = concatenated_differences

        # --- FIX START ---
        # 1. Always reshape to the NATIVE size first (matches CLIP embedding math)
        native_h, native_w = 256, 384
        ccl_embedding = concatenated_differences_padded.reshape(frame, native_h, native_w)
        
        # 2. Add channel dimension
        ccl_embedding = ccl_embedding.unsqueeze(1)  # Shape: [Frame, 1, 256, 384]

        # 3. Interpolate (resize) to your TARGET sample_size (e.g., 512x512)
        if (self.sample_size[0] != native_h) or (self.sample_size[1] != native_w):
            ccl_embedding = F.interpolate(
                ccl_embedding, 
                size=(self.sample_size[0], self.sample_size[1]), 
                mode='bilinear', 
                align_corners=False
            )
        # --- FIX END ---
        ...
        # Generate the Color Temp map at the TARGET size directly
...
def run_inference(pipeline, tokenizer, text_encoder, base_scene, color_temperature_list, output_dir, device, video_length=5, height=512, width=512):
    ...
    # Ensure camera_embedding is on the correct device AND passes the custom sample_size
    camera_embedding = Camera_Embedding(color_temperature_values, tokenizer, text_encoder, device, sample_size=[height, width]).load()
...
def main(config_path, base_scene, color_temperature_list, height, width):
    ...
    run_inference(pipeline, pipeline.tokenizer, pipeline.text_encoder, base_scene, color_temperature_list, cfg.output_dir, device=device, height=height, width=width)
...
if __name__ == "__main__":
    ...
    main(args.config, args.base_scene, args.color_temperature_list, args.height, args.width)
