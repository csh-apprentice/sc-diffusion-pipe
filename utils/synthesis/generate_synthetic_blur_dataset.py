...
- Absolute timeline: you provide --duration_s and --samples_per_exposure.
- For FPS f and 360° shutter, frame k integrates over [k/f, (k+1)/f], sampled uniformly.
- Two timeline modes:
  * per_fps        : each FPS uses its own (k+0.5)/f mid-times → #frames ≈ duration_s * f
...
def frame_mid_times_per_fps(duration_s: float, fps: int) -> List[float]:
    """Mid-exposure times t_k = (k+0.5)/fps within [0, duration_s], with full window inside duration."""
    times = []
    k = 0
    while True:
        start = k / float(fps)
        ...
        if end > duration_s + 1e-9:
            break
        times.append(start + 0.5 / float(fps))
        k += 1
    return times

def frame_mid_times_align_min(duration_s: float, min_fps: int) -> List[float]:
    """Same mid-times for all fps, based on smallest fps."""
    return frame_mid_times_per_fps(duration_s, min_fps)
...
def main():
    ap = argparse.ArgumentParser("Synthetic FPS/Shutter dataset generator (absolute-time)")
    ...
    ap.add_argument("--duration_s", type=float, default=3.0,
                    help="Total simulated duration in seconds (e.g., 1.0).")
    ...
    if do_video:
        ...
        print(f"    Timeline mode: {args.timeline} (duration={args.duration_s}s, samples/exposure={args.samples_per_exposure})")
    ...
    for idx in pbar:
        ...
        if do_image:
            ...
            # Render a single image at the mid-time of the whole duration
            t_mid = 0.5 * args.duration_s
            for fps in fps_values:
                exposure_span = 1.0 / float(fps)
                # Build a local copy, advance to t_mid, integrate over exposure window centered at t_mid.
                # But integration window for 360° shutter is [t_mid - 1/(2f), t_mid + 1/(2f)] clamped to [0, duration].
        ...
        if do_video:
            ...
            base_times = (frame_mid_times_align_min(args.duration_s, min_fps)
                          if args.timeline == "align_min_fps"
                          else None)
            ...
            for fps in fps_values:
                ...
                mid_times = base_times if base_times is not None else frame_mid_times_per_fps(args.duration_s, fps)
                ...
                for t_mid in mid_times:
                    t_start = max(0.0, t_mid - 0.5 * exposure_span)  # equals k/f in per_fps mode
                    # advance to the start of the exposure window
