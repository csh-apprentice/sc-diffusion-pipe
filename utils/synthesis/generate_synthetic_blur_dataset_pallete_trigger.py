...
- Selects background and shape colors from a fixed palette, ensuring minimum perceptual contrast.
...
def sample_scene(W: int,
    ...
    for i in range(n):
        ...
        color_name, color_rgb = random.choice(COLOR_PALETTE) # <--- MODIFIED
        ...
        if make_static:
            vx, vy = 0.0, 0.0
        shapes.append(Shape(kind, color_name, color_rgb, x, y, vx, vy, size)) # <--- MODIFIED
    return shapes
...
COLOR_LAB_CACHE = {name: rgb_to_lab(rgb) for name, rgb in COLOR_PALETTE} # <--- NEW

def choose_background(shapes: List[Shape],
                      min_delta_e: float = 30.0) -> Tuple[str, Tuple[int,int,int]]: # <--- MODIFIED
    shape_labs = [rgb_to_lab(s.color_rgb) for s in shapes]
    
    # Shuffle the palette to try different backgrounds
    shuffled_palette = random.sample(COLOR_PALETTE, len(COLOR_PALETTE)) # <--- NEW
    
    for bg_name, bg_rgb in shuffled_palette: # <--- MODIFIED
        bg_lab = COLOR_LAB_CACHE[bg_name] # <--- MODIFIED
        if all(delta_e_cie76(bg_lab, s_lab) >= min_delta_e for s_lab in shape_labs):
            return bg_name, bg_rgb # <--- MODIFIED

    warnings.warn("Could not find a background with sufficient contrast; defaulting to white.")
    return "white", (255, 255, 255) # <--- MODIFIED
...
def main():
    ...
    for idx in pbar:
        ...
        # Now returns both name and rgb
        bg_color_name, bg_rgb = choose_background(base_shapes_for_bg, min_delta_e=args.bg_min_delta_e) # <--- MODIFIED
        ...
        if do_image:
            ...
            for s_val, fps in zip(scales, fps_per_scale):
                ...
                if args.caption_blur_note or args.caption_relations or args.trigger_phrase:
                    cap = make_caption(
                        shapes_img_base,
                        bg_color_name=bg_color_name, # <--- NEW
