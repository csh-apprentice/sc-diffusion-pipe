...
- Selects shape colors from a fixed palette; background is fully randomized.
...
def sample_scene(W: int,
    ...
    for i in range(n):
        ...
        color_name, color_rgb = random.choice(COLOR_PALETTE) # Shape color from palette
        ...
        if make_static:
            vx, vy = 0.0, 0.0
        shapes.append(Shape(kind, color_name, color_rgb, x, y, vx, vy, size))
    return shapes
...
COLOR_LAB_CACHE = {name: rgb_to_lab(rgb) for name, rgb in COLOR_PALETTE}

def choose_background(shapes: List[Shape],
                      min_delta_e: float = 30.0) -> Tuple[int,int,int]: # <--- MODIFIED
    # Get LAB values for the shapes (which are from the palette)
    shape_labs = [COLOR_LAB_CACHE[s.color_name] for s in shapes] # <--- MODIFIED
    
    for _ in range(100): # Try 100 times to find a contrasting *random* background
        # Generate a fully random background color
        bg_rgb = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)) # <--- MODIFIED
        bg_lab = rgb_to_lab(bg_rgb) # <--- MODIFIED
        
        # Check contrast against all shapes
        if all(delta_e_cie76(bg_lab, s_lab) >= min_delta_e for s_lab in shape_labs):
            return bg_rgb # <--- MODIFIED

    warnings.warn("Could not find a background with sufficient contrast; defaulting to white.")
    return (255, 255, 255) # <--- MODIFIED
...
def make_caption(shapes: List[Shape],
    ...
    for s in shapes:
        ...
        # Includes the shape color name from the palette
        summary.append(f"a {s.color_name} {mv} {s.kind} at the {pos}" if include_relations # <--- MODIFIED
                       else f"a {s.color_name} {mv} {s.kind}")
    
    who = ", ".join(summary)
    
    # Background text is now generic
    base = f"{'Video' if for_video else 'Image'} of {who} on a background. " # <--- MODIFIED
...
def main():
    ...
    for idx in pbar:
        ...
        # Now only returns rgb
        bg_rgb = choose_background(base_shapes_for_bg, min_delta_e=args.bg_min_delta_e) # <--- MODIFIED
        ...
        if do_image:
            ...
            for s_val, fps in zip(scales, fps_per_scale):
                ...
                if args.caption_blur_note or args.caption_relations or args.trigger_phrase:
                    cap = make_caption(
                        shapes_img_base,
                        # bg_color_name removed
                        fps=fps, exposure_factor=1.0,
